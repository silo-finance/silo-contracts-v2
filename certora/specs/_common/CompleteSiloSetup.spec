...
import "./HookReceiverMethodsDispatch.spec";
...
function completeSiloSetupAddress(address sender)
{
    ...
    //doesntHaveCollateralAsWellAsDebt(sender); // no longer apply 

    // add require sender != liquidationModule when needed    
    ...
    require shareDebtToken0.totalSupply() == 0 <=> silo0.total(require_uint256(ISilo.AssetType.Debt)) == 0;  
    require shareProtectedCollateralToken0.totalSupply() == 0 <=> silo0.total(require_uint256(ISilo.AssetType.Protected)) == 0;
    require shareCollateralToken0.totalSupply() == 0 <=> silo0.total(require_uint256(ISilo.AssetType.Collateral)) == 0;

    require shareDebtToken1.totalSupply() == 0 <=> silo1.total(require_uint256(ISilo.AssetType.Debt)) == 0;
    require shareProtectedCollateralToken1.totalSupply() == 0 <=> silo1.total(require_uint256(ISilo.AssetType.Protected)) == 0;
    require shareCollateralToken1.totalSupply() == 0 <=> silo1.total(require_uint256(ISilo.AssetType.Collateral)) == 0;
...
function totalSupplyMoreThanBalance(address receiver)
{
    ...
    require token0.totalSupply() >= require_uint256(
        silo0.total(require_uint256(ISilo.AssetType.Protected)) + silo0.total(require_uint256(ISilo.AssetType.Collateral)) + token0.balanceOf(receiver));
    require token1.totalSupply() >= require_uint256(
        silo1.total(require_uint256(ISilo.AssetType.Protected)) + silo1.total(require_uint256(ISilo.AssetType.Collateral)) + token1.balanceOf(receiver));

    // if user has debt he must have collateral in the other silo (in the initial state)
    ...
    // total supplies of the tokens are reasonable (in the initial state)
    require token0.totalSupply() < max_uint128;
    require token1.totalSupply() < max_uint128;
    require shareDebtToken0.totalSupply() < max_uint128;
    require shareDebtToken1.totalSupply() < max_uint128;
    require shareCollateralToken0.totalSupply() < max_uint128;
    require shareCollateralToken1.totalSupply() < max_uint128;
    require shareProtectedCollateralToken0.totalSupply() < max_uint128;
    require shareProtectedCollateralToken1.totalSupply() < max_uint128;
}

// DO NOT USE THIS. This requirement no longer applies since v7.0
...
// initial state is treated differently. (When there's 0 collateral or debt shares)
function requireNotZeroState()
{
...
definition canIncreaseAccrueInterest(method f) returns bool =
    ...
    f.selector == sig:borrow(uint256,address,address,bool).selector ||
    f.selector == sig:borrowShares(uint256,address,address,bool).selector ||
    ...
    f.selector == sig:leverage(uint256,address,address,bool,bytes).selector ||
    // f.selector == sig:liquidationCall(address,address,address,address,uint256,bool).selector ||
    ...
    f.selector == sig:switchCollateralTo(bool).selector ||
    f.selector == sig:liquidationRepay(uint256,address,address).selector ||
...
definition mustCallCrossNonReentrant(method f) returns bool =
    (canIncreaseSharesBalance(f) ||
    canDecreaseSharesBalance(f) ||
    canIncreaseTotalDebt(f) ||
    canDecreaseTotalDebt(f)) && 
    f.selector != sig:withdrawCollateralsToLiquidator(uint256,uint256,address,address,bool).selector &&
    f.selector != sig:withdrawFees().selector;
