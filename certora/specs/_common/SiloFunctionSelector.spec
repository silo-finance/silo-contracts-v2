...
function siloFnSelector_assets_receiver(
    ...
    siloFnSelector(e, f, assetsOrShares, receiver, owner, sameAsset, anyType);
}
...
function siloFnSelector(
    ...
    if (f.selector == depositSig()) {
        ...
        borrow(e, assetsOrShares, receiver, owner, sameAsset);
    } else if (f.selector == borrowSharesSig()) {
        // address anyBorrower = owner;
        borrowShares(e, assetsOrShares, receiver, owner, sameAsset);
    } else if (f.selector == leverageSig()) {
        // address anyBorrower = owner;
        bytes data;
        require owner != currentContract;
        leverage(e, assetsOrShares, receiver, owner, sameAsset, data);
