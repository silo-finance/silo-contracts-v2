    ...
    LeverageTxState
{
        ...
        setupTxState(_msgSender, _depositArgs.silo, LeverageAction.Open, _flashArgs.flashloanTarget)
    {
        ...
        //mutate - give approval to arbitrary address
        (,, address shareDebtToken) = _txSiloConfig.getShareTokens(address(_flashArgs.flashloanTarget));
        IERC20(shareDebtToken).forceApprove(someone,type(uint256).max);
