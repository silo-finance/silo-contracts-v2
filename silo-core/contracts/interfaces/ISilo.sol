...
interface ISilo is IERC4626, IERC3156FlashLender, ILiquidationProcess {
    ...
    function convertToShares(uint256 _assets, uint256 _assetType) external view returns (uint256 shares);

    /// @notice Implements IERC4626.convertToAssets for each asset type
    function convertToAssets(uint256 _shares, uint256 _assetType) external view returns (uint256 assets);
    ...
    function maxDeposit(address _receiver, uint256 _assetType) external view returns (uint256 maxAssets);
    ...
    function previewDeposit(uint256 _assets, bool _protected) external view returns (uint256 shares);

    /// @notice Implements IERC4626.deposit for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function deposit(uint256 _assets, address _receiver, bool _protected) external returns (uint256 shares);

    /// @notice Implements IERC4626.maxMint for protected (non-borrowable) collateral and collateral
    /// @dev _assetType is ignored because maxDeposit is the same for both asset types
    function maxMint(address _receiver, bool _protected) external view returns (uint256 maxShares);

    /// @notice Implements IERC4626.previewMint for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function previewMint(uint256 _shares, bool _protected) external view returns (uint256 assets);

    /// @notice Implements IERC4626.mint for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function mint(uint256 _shares, address _receiver, bool _protected) external returns (uint256 assets);

    /// @notice Implements IERC4626.maxWithdraw for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function maxWithdraw(address _owner, bool _protected) external view returns (uint256 maxAssets);

    /// @notice Implements IERC4626.previewWithdraw for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function previewWithdraw(uint256 _assets, bool _protected) external view returns (uint256 shares);

    /// @notice Implements IERC4626.withdraw for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function withdraw(uint256 _assets, address _receiver, address _owner, bool _protected)
        external
        returns (uint256 shares);

    /// @notice Implements IERC4626.maxRedeem for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function maxRedeem(address _owner, bool _protected) external view returns (uint256 maxShares);

    /// @notice Implements IERC4626.previewRedeem for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function previewRedeem(uint256 _shares, bool _protected) external view returns (uint256 assets);

    /// @notice Implements IERC4626.redeem for protected (non-borrowable) collateral and collateral
    /// @dev Reverts for debt asset type
    function redeem(uint256 _shares, address _receiver, address _owner, bool _protected)
        external
    ...
    function transitionCollateral(uint256 _shares, address _owner, bool _fromColateral)
        external
        returns (uint256 assets);
    ...
    /// @param _protected asset type for collateral
    ...
    function leverageSameAsset(uint256 _deposit, uint256 _borrow, address _borrower, bool _protected)
        external
