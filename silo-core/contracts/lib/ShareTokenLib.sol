...
library ShareTokenLib {
    ...
    function afterTokenTransfer(address _sender, address _recipient, uint256 _amount) internal {
        ...
        uint256 action = Hook.shareTokenTransfer($.hookSetup.tokenType);

        if (!$.hookSetup.hooksAfter.matchAction(action)) return;
        ...
        IHookReceiver($.hookSetup.hookReceiver).afterAction(
            address($.silo),
            action,
            abi.encodePacked(
                _sender,
