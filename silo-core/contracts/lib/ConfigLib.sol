        ...
        returns (uint256 order)
    {
        if (!_debtInfo.debtPresent) {
            if (_hook & Hook.BORROW & Hook.SAME_ASSET != 0) {
                return _callForSilo0 ? SILO_SILO0 : SIL1_SILO1;
            } else if (_hook & Hook.BORROW & Hook.TWO_ASSETS != 0) {
                return _callForSilo0 ? SIL1_SILO0 : SILO_SILO1;
            } else {
               return _callForSilo0 ? SILO_SILO1 : SIL1_SILO0;
            }
            ...
            if (_debtInfo.sameAsset) {
                if (_debtInfo.debtInSilo0) {
                    return _callForSilo0 ? SILO_SILO0 : SIL1_SILO0 /* only deposit */;
                } else {
                    return _callForSilo0 ? SILO_SILO1 /* only deposit */ : SIL1_SILO1;
                }
            } else {
                if (_debtInfo.debtInSilo0) {
                    return _callForSilo0 ? SILO_SILO1 : SIL1_SILO0 /* only deposit */;
                } else {
                    return _callForSilo0 ? SILO_SILO1 /* only deposit */ : SIL1_SILO0;
                }
            }
        }
        ...
        if (_debtInfo.debtInSilo0) {
            ...
            return _debtInfo.sameAsset ? SILO_SILO0 : SIL1_SILO0;
        } else {
            _debtInfo.debtInThisSilo = !_callForSilo0;
            return _debtInfo.sameAsset ? SIL1_SILO1 : SIL0_SILO1;
        }
    }
}
