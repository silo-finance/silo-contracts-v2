...
library SiloStorageLib {
    ...
    function siloConfig() internal view returns (ISiloConfig config) {
        config = getSiloStorage().sharedStorage.siloConfig;
    }

    function hookReceiver() internal view returns (IHookReceiver receiver) {
        receiver = getSiloStorage().sharedStorage.hookReceiver;
    }
}
