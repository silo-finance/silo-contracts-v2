...
import {ERC20, ERC20Permit, IERC20Permit} from "openzeppelin5/token/ERC20/extensions/ERC20Permit.sol";
...
// TODO: use ERC20PermitUpgradable
/// @title SiloERC4626
/// @notice Implements common interface for ERC4626 and Silo share token representing collateral.
...
abstract contract SiloERC4626 is ISilo, SiloStorage, ERC20Permit {
    using Hook for uint24;
    using CallBeforeQuoteLib for ISiloConfig.ConfigData;

    string private constant _NAME = "SiloShareToken";
    uint24 private constant collateralTokenType = uint24(Hook.COLLATERAL_TOKEN);
    ...
    function __ShareToken_init() internal virtual {
        transferWithChecks = true;
    }

    function _burn(address _owner, address _spender, uint256 _amount) internal {
        if (_owner != _spender) _spendAllowance(_owner, _spender, _amount);
        _burn(_owner, _amount);
    }

    function forwardTransferFromNoChecks(address _from, address _to, uint256 _amount) external virtual {
    ...
    /// @inheritdoc IERC20
    function approve(address _spender, uint256 _amount) public override(ERC20, IERC20) returns (bool result) {
        ISiloConfig siloConfig = _getSiloConfig();

        siloConfig.turnOnReentrancyProtection();

        result = ERC20.approve(_spender, _amount);
    ...
    /// @inheritdoc IERC20
    function transfer(address _to, uint256 _amount) public override(ERC20, IERC20) returns (bool result) {
        ISiloConfig siloConfig = _getSiloConfig();
    ...
    /// @inheritdoc IERC20
    function transferFrom(address _from, address _to, uint256 _amount) public override(ERC20, IERC20) returns (bool result) {
        ISiloConfig siloConfig = _getSiloConfig();
    ...
    // TODO: Any positive/negative consequences of share token having the same decimals as asset?
    /// @inheritdoc IERC20Metadata
    // function decimals() public view virtual override(ERC20, IERC20Metadata) returns (uint8) {
    //     ISiloConfig.ConfigData memory configData = _getSiloConfig().getConfig(address(this));
    ...
    function _afterTokenTransfer(address _sender, address _recipient, uint256 _amount) internal virtual {
        ...
        uint256 action = Hook.shareTokenTransfer(collateralTokenType);

        if (!_getSharedStorage().hooksAfter.matchAction(action)) return;
        ...
        IHookReceiver(_getSharedStorage().hookReceiver).afterAction(
            address(this),
            action,
    ...
    function _isSolventAfterCollateralTransfer(address _borrower) internal virtual returns (bool) {
        ...
        ) = _getSiloConfig().getConfigsForWithdraw(address(this), _borrower);

        // when deposit silo is collateral silo, that means this sToken is collateral for debt
        if (collateralConfig.silo != depositConfig.silo) return true;

        collateralConfig.callSolvencyOracleBeforeQuote();
        debtConfig.callSolvencyOracleBeforeQuote();

        return SiloSolvencyLib.isSolvent(collateralConfig, debtConfig, _borrower, ISilo.AccrueInterestInMemory.Yes);
    }
    ...
    function _getSiloConfig() internal view virtual returns (ISiloConfig siloConfig);
    function _getSharedStorage() internal view virtual returns (ISilo.SharedStorage storage sharedStorage);
