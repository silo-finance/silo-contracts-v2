...
contract Silo is Initializable, SiloERC4626, ReentrancyGuardUpgradeable, LeverageReentrancyGuard {
    ...
    /// @inheritdoc ISilo
    function getLiquidity(AccrueInterestInMemory _accrueInterest) public view virtual returns (uint256 liquidity) {
        if (_accrueInterest == AccrueInterestInMemory.Yes) {
            ISiloConfig.ConfigData memory config = config.getConfig(address(this));
    ...
    function maxDeposit(address _receiver) external view virtual returns (uint256 maxAssets) {
        return SiloERC4626Lib.maxDeposit(config, _receiver, siloData.daoAndDeployerFees, _total);
    }
        ...
        returns (uint256 assets)
    {
        ...
        { // Stack too deep
            (address shareTokenFrom, uint256 liquidity) = _withdrawType == AssetType.Collateral
                ? (configData.collateralShareToken, getLiquidity(AccrueInterestInMemory.No))
    ...
    function withdrawCollateralsToLiquidator(
        ...
        SiloLiquidationExecLib.withdrawCollateralsToLiquidator(
            ...
            _liquidator,
            _receiveSToken,
            getLiquidity(AccrueInterestInMemory.No),
            _total
        );
    }
