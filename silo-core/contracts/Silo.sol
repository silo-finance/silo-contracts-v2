...
contract Silo is Initializable, SiloERC4626, ReentrancyGuardUpgradeable, LeverageReentrancyGuard {
    ...
    function utilizationData() external view virtual returns (UtilizationData memory) {
        ...
        return UtilizationData({
            collateralAssets: uint128(_total.collateral),
            debtAssets: uint128(_total.debt),
            interestRateTimestamp: siloData.interestRateTimestamp
        });
    }

    function getLiquidity() public view virtual returns (uint256 liquidity) {
        liquidity = SiloMathLib.liquidity(_total.collateral, _total.debt);
    }
    ...
    {
        totalCollateralAssets = _total.collateral;
        totalProtectedAssets = _total.protected;
    }
    ...
    function maxWithdraw(address _owner) external view virtual returns (uint256 maxAssets) {
        (maxAssets,) = SiloERC4626Lib.maxWithdraw(
            config, _owner, AssetType.Collateral, _total.collateral, getLiquidity()
        );
    }
    ...
    function maxWithdraw(address _owner, AssetType _assetType) external view virtual returns (uint256 maxAssets) {
        (maxAssets,) =
            SiloERC4626Lib.maxWithdraw(
                config, _owner, _assetType, _assetType == AssetType.Collateral ? _total.collateral : _total.protected, getLiquidity()
            );
    }
        ...
        returns (uint256 assets)
    {
        ...
        { // Stack too deep
            (address shareTokenFrom, uint256 liquidity) = _withdrawType == AssetType.Collateral
                ...
                : (configData.protectedShareToken, _total.protected);
    ...
    function liquidationCall(
        ...
        SiloLendingLib.repay(debtConfig, repayDebtAssets, zeroShares, _borrower, msg.sender, _total);
        ...
        returns (uint256 assets, uint256 shares)
    {
        ...
        (assets, shares) =
            SiloLendingLib.repay(configData, _assets, _shares, _borrower, msg.sender, _total);
