...
contract SiloIntegrationTest is VeSiloFeatures {
    ...
    function test_anvil_VeSiloWithSiloCoreAndSiloOracles() public {
        ...
        _configureIncentiveHook(hookReceiver, IShareToken(shareToken), gauge);
        _addGauge(gauge);
        _voteForGauge(gauge);
    ...
    function _configureIncentiveHook(address _hookReceiver, IShareToken _shareToken, address _gauge) internal {
        ...
        calldatas[0] = abi.encodeCall(IIncentiveHook.addNotificationReceiver, (_shareToken, INotificationReceiver(_gauge)));

        address[] memory notificationReceivers = IIncentiveHook(_hookReceiver).getNotificationReceivers(_shareToken);

        assertEq(notificationReceivers.length, 0, "Hook receiver should not have notification receivers");

        _executeProposal(targets, values, calldatas);

        notificationReceivers = IIncentiveHook(_hookReceiver).getNotificationReceivers(_shareToken);

        assertEq(notificationReceivers.length, 1, "Hook receiver should have one notification receiver");
        assertEq(notificationReceivers[0], _gauge, "Hook receiver should have the gauge as notification receiver");
    }
