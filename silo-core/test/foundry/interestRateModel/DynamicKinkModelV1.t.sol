...
contract DynamicKinkModelV1Test is RcompTestDynamicKink, RcurTestDynamicKink {
    ...
    function test_rcur() public {
        ...
        for (uint i; i < data.length; i++) {
            ...
            relativeCheck("relative error for rcur in 10^18 bp new/expected", rcur, data[i].expected.currentAnnualInterest);
            relativeCheck("relative error for k in 10^18 bp new/expected", k, data[i].debug.k);
            relativeCheck("relative error for r in 10^18 bp new/expected",r, data[i].debug.r);
            emit log_string("******\n\n\n\n");
        }
    ...
    function relativeCheck(string memory message, int256 a, int256 b) internal {
        if (b != 0) {
            emit log_named_int(message, a * DP / b);
        } else {
            emit log_named_int(message, a == b ? DP : int256(0));
        }
    }
    ...
    function test_rcomp() public {
        ...
        for (uint i; i < data.length; i++) {
            ...
            relativeCheck("relative error for rcomp in 10^18 bp new/expected", rcomp, data[i].expected.compoundInterest);
            relativeCheck("relative error for k in 10^18 bp new/expected", k, data[i].expected.newSlope);
            relativeCheck("relative error for x in 10^18 bp new/expected", x, data[i].debug.x);
