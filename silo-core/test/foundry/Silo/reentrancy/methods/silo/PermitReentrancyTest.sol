...
contract PermitReentrancyTest is MethodReentrancyTest {
    ...
    function verifyReentrancy() external {
        _ensureItWillRevertReentrancy(address(TestStateLib.silo0()));
        _ensureItWillRevertReentrancy(address(TestStateLib.silo1()));
    }

    function _ensureItWillRevertReentrancy(address _silo) internal {
        VmSafe.Wallet memory signer = vm.createWallet("Proof signer");
        address spender = makeAddr("Spender");
        uint256 value = 100e18;
        uint256 nonce = ShareToken(_silo).nonces(signer.addr);
        uint256 deadline = block.timestamp + 1000;

        (uint8 v, bytes32 r, bytes32 s) =
            _createPermit(signer, spender, value, nonce, deadline, address(_silo));

        vm.expectRevert(ICrossReentrancyGuard.CrossReentrantCall.selector);
        ShareToken(_silo).permit(signer.addr, spender, value, deadline, v, r, s);
    }
    ...
    function _ensureItWillNotRevert(address _silo) internal {
        VmSafe.Wallet memory signer = vm.createWallet("Proof signer");
        address spender = makeAddr("Spender");
        uint256 value = 100e18;
        uint256 nonce = ShareToken(_silo).nonces(signer.addr);
        ...
        SiloERC4626(_silo).permit(signer.addr, spender, value, deadline, v, r, s);
    }
