...
contract Hack1 {
        ...
        returns (bytes32)
    {
        ...
        if (option == 0) {
            ...
            Silo(msg.sender).borrow(assets, receiver, _initiator, sameToken);
        } else if (option == 6) {
            Silo(msg.sender).borrowShares(shares, receiver, _initiator, sameToken);
        } else if (option == 7) {
            Silo(msg.sender).repay(assets, _initiator);
        } else if (option == 8) {
            Silo(msg.sender).repayShares(shares, _initiator);
        } else {
            Silo(msg.sender).leverage(assets, ILeverageBorrower(receiver), _initiator, sameToken, bytes(""));
        }
