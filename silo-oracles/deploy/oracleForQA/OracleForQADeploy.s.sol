...
/**
ETHERSCAN_API_KEY=$ARBISCAN_API_KEY FOUNDRY_PROFILE=oracles QUOTE=0xaf88d065e77c8cC2239327C5EDb3A432268e5831 \
...
contract OracleForQADeploy is CommonDeploy {
    function run() public returns (OracleForQA oracle) {
        ...
        string memory symbol = IERC20Metadata(quote).symbol();

        vm.startBroadcast(deployerPrivateKey);

        oracle = new OracleForQA(quote);

        vm.stopBroadcast();

        OraclesDeployments.save(getChainAlias(), string.concat("OracleForQA_",symbol), address(oracle));
    }
}
