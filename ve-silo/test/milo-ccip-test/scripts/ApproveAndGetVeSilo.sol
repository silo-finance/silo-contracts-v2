 ...
 */
contract ApproveAndGetVeSilo is Script {
    function run() external returns (uint256 votingPower) {
        AddrLib.init();
        ...
        uint256 balance = IERC20(miloToken).balanceOf(proposer);

        vm.startBroadcast(proposerPrivateKey);

        IERC20(miloToken).approve(veSilo, type(uint256).max);
        IVeSilo(veSilo).create_lock(balance, block.timestamp + 300 days);

        vm.stopBroadcast();

        votingPower = IERC20(veSilo).balanceOf(proposer);
    }
}
